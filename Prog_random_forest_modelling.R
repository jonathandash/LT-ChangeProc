...
# Read predictors
gain.preds_bal<-readRDS(here('out', 'lt_gain.predictors_bal.rds'))
loss.preds_bal<-readRDS(here('out', 'lt_loss.predictors_bal.rds'))
gain.preds_cons<-readRDS(here('out', 'lt_gain.predictors_cons.rds'))
loss.preds_cons<-readRDS(here('out', 'lt_loss.predictors_cons.rds'))
...
ref.loss.cons<-st_intersection(ref.loss, loss_shp_cons)
st_geometry(ref.loss.cons) <- NULL

ref.loss.bal<-st_intersection(ref.loss, loss_shp_bal)
st_geometry(ref.loss.bal) <- NULL


ref.gain.cons<-st_intersection(ref.gain, gain_shp_cons)
st_geometry(ref.gain.cons) <- NULL

ref.gain.bal<-st_intersection(ref.gain, gain_shp_bal)
st_geometry(ref.gain.bal) <- NULL

# Add predictors into reference  dataset
ref.gain.bal<- left_join(ref.gain.bal, gain.preds_bal, by = 'fid')
ref.gain.cons<- left_join(ref.gain.cons, gain.preds_cons, by = 'fid')

ref.loss.bal<- left_join(ref.loss.bal, loss.preds_bal, by = 'fid')
ref.loss.cons<- left_join(ref.loss.cons, loss.preds_cons, by = 'fid')
